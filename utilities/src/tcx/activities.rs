...
mod tests {
    ...
    fn test_activities_summary_new() {
        ...
        act.total_time_seconds = Some(1800.0);
        ...
        assert!(act.total_time_seconds.is_some());
        ...
        assert_eq!(act.total_time_seconds.unwrap(), 1800.0);
        assert_eq!(
            act.notes.unwrap(),
    ...
    fn test_from_activities() {
        ...
        if let Some(activities) = tcdb.activities {
            act = TCXActivitiesSummary::from_activities(&activities);
            println!("act = {:?}", act);
        }
        ...
        assert_eq!(act.total_time_seconds.unwrap(), 1325.444009);
        ...
        assert_eq!(act.average_speed.unwrap(), 2.235717863507277);
